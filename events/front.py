...
def front(temp_id, input_image_raw=None, input_image=None, front_components=None, others=None, advanced_components=None, true=None, false=None, demo=None, difficult=None, download_share_buttons=None, raws = None, download_bar = None):
    ...
    all_components = [temp_id, input_image_raw, artist, season, classes, member, numbering_state, number, alphabet, serial, qr_code]

    if not any(component == '' for component in [input_image_raw, artist]):
        for component in all_components:
            if component == artist:
                component.input(fn=lambda x, y: make_json(x, y) + [False], inputs=[temp_id, input_image_raw, artist],
    ...
    input_image.upload(fn=resize_round, inputs=[input_image] + all_components,
                       outputs=[temp_id, input_image, input_image_raw, download_front, download_back, download_combine, front_raw, back_raw, combined_raw]+ advanced_components)
