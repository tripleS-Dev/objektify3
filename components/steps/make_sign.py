...
def make_sign(walkthrough, members, members_step, sign_upload):

    with gr.Row():
        members_radio = gr.Radio(label='Member', info='Select member to edit', interactive=True, choices=['aaa', 'bb'])
        progress_checkbox = gr.CheckboxGroup(interactive=False, visible=True, choices=None, label='Progress', info='You must complete all signatures to proceed.1')


    with gr.Row():


        with gr.Row(elem_classes='sticky-image-small', visible=True):
        ...
        with gr.Column():
            ...
            sign_file = gr.File(file_count='single', file_types=['.png', '.webp', '.svg', '.jpeg', '.jpg'], interactive=True)
            ...
            progress_checkbox.change(inputs=[progress_checkbox, members], outputs=btn4, fn=lambda x, y: gr.Button(interactive=True, variant='primary') if len(x) == len(y) else gr.Button(interactive=False, variant='secondary'))
            ...
            sign_file.upload(fn=sign_upload, inputs=[members, members_radio, sign_save, sign_file, sing_x, sing_y, sing_size, remove_bg, progress_checkbox], outputs=[apply_btn, sign_preview, sign_save, progress_checkbox])#여기
            ...
            apply_btn.click(fn=sign_upload, inputs=[members, members_radio, sign_save, sign_file, sing_x, sing_y, sing_size, remove_bg, progress_checkbox], outputs=[apply_btn, sign_preview, sign_save, progress_checkbox])